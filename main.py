...
def amortization1(C: float, T: int, L: float) -> Tuple[float, List[float]]:
  ...
  """
  Am = (C - L) / T
  C_history = []  # Иницицилизация истории стоимости объекта
  for _ in range(T):
    C -= Am
    C_history.append(round(C, 2))
  return Am, C_history # Можно посмотреть результаты выполнения функции и ее возврат
...
Am, C_history = amortization1(C=30000, T=8, L=0)
print(f"Am = {Am}\nC_history = {C_history}")
...
Am_history_2, C_history_2 = amortization2(C=30000, T=8, k=2)
